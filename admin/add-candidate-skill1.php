...
extract($_POST);
/*foreach($_POST['syear'] as $movie){
    if(!empty($movie)){
   echo $movie . "<br />"; }
}*/
// foreach($_POST['syear'] as $year_id){ var_dump($year_id); } exit();
if(count($_POST["sname"]) > 0){
    ...
    $sql_prd="INSERT into skills(emp_id,skill_name) values ('$emp_id','$sname')";
    if(mysqli_query($conn, $sql_prd)) {
     $last_id[]= mysqli_insert_id($conn); }
  }
 } 
   foreach($_POST['syear'] as $key => $years){
        echo $did = $last_id[$key]; 
        //if(!empty($years)){
          if (empty($_POST[$years])) {
        $sql1 ="UPDATE skills SET skill_year='$years' WHERE id='$did'"; 
...
/*if(count($_POST["sname"]) > 0){
    ...
    $sname = mysqli_real_escape_string($conn,$_POST['sname'][$i]);
    $syear = mysqli_real_escape_string($conn,$_POST['syear'][$i]);
   foreach($_POST['syear'] as $years){
    if(!empty($years)){
    $sqlsr =mysqli_query($conn,"SELECT count(*) as vd FROM skills WHERE emp_id = '".$emp_id."' and skill_name='".$sname."' and skill_year='".$years."'");
    $rescd= mysqli_fetch_array($sqlsr);
    if($rescd['vd'] == 0){
    $sql_prd="INSERT into skills(emp_id,skill_name,skill_year) values ('$emp_id','$sname','$years')";
    if(mysqli_query($conn, $sql_prd)) {
     $last_id[]= mysqli_insert_id($conn); }
    }
   }
