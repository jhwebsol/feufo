...
$id=$_POST["id"];
$sql1 ="UPDATE  policy  SET data='$datas'  WHERE id='$id'"; 
$res=mysqli_query($conn,$sql1);
    if($res){
$sqlg = "SELECT * from policy where id = '$id'";
  ...
  if($_FILES["ban_img"]["name"] != ""){
    ...
    $path = "banner/".$resg->id.'po'.'.'.$extension;
    $upath = "banner/".$resg->image;
    unlink($upath);
     move_uploaded_file($tn,$path);
      $image = $resg->id.'po'.'.'.$extension;
    } else {
      $image = $resg->image;
    }
$sqlup = "UPDATE policy SET  image =  '$image'  WHERE  id = $resg->id";
