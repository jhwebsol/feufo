	...
	<div class="wrapper">
     ...
     <div class="box box-danger gurnew">
        <div class="box-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">Candidate Interview Detail</h6>
        </div>
							<div class="box-body"> 
								<div class="table-responsive">
									 <table class="table table-bordered dataTable no-footer example1" style="width: 100%;" cellspacing="0">
										...
										<tbody>
											<?php $j=1; 
											$sql="select employee_details.emp_name,employer.company_name,employer.emp_name as empr,cdn.* from candidate_interview_details cdn join employee_details on cdn.emp_id=employee_details.id join employer on cdn.empr_id=employer.id "; 
                                        ...
                                        <select class="toggle btn btn-success nstatus" id="nstatus" data-toggle="toggle" role="button" style="width: 96.65px; height: 37.6px;">
                                            ...
                                            <option value="Inactive" data-nid="<?= $res['id'];?>" for="" class="btn btn-success toggle-on">InActive</option>
											    ...
											    <td> <a href="view-condidate-interview.php?id=<?php echo $res['id']; ?>" class="btn btn-success btn-sm"><i class="fa fa-eye fa-lg"></i></a>
                                                <a href="javascript:delete_empr_by_ID('<?php echo $res['id'] ?>');" class="btn btn-danger btn-sm"><i class="fa fa-trash fa-lg"></i></a></td>										  
											</tr>
										<?php } ?>
										</tbody>
										 </form>
...
function delete_empr_by_ID(id)
{
    if (confirm('Do You Want to Deleting This \nContinue anyway?')) {
        window.location.href = 'delete_intcandidate.php?id=' + id;
    }
}
$(document).ready(function(){
$('.nstatus').on('change', function(){
    ...
    var dbdetails="candidate_interview_details";
    if(nstatus){
        $.ajax({
            type:'POST',
